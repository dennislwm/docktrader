...
import sys
get_ipython().system('{sys.executable} -m pip install couchdb')
import wget
import pandas as pd
import os
import datetime
...
dbServer = couchdb.Server('http://localhost:5984/')
print(dbServer)
if ("dbStockRow" in dbServer) == False:
    db=dbServer.create('dbStockRow')
else:
    db=dbServer['dbStockRow']
